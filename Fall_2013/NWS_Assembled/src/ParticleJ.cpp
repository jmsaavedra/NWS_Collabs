...
void ParticleJ::setup(ofVec2f Pos, float Vel,float hue){
    pos = Pos;
...
void ParticleJ::passValues(float *_xNoiseValue, float *_yNoiseValue, float *_zNoiseValue, float *_xNoiseScale, float *_yNoiseScale, float *_xAngleOffset,float *_yAngleOffset) {
...
void ParticleJ::update(float vecScale){
...
void ParticleJ::draw(){
    ofSetColor(c,100);
    ofCircle(pos.x, pos.y, 10 * 1.5*(1-(age/lifeTime)));
